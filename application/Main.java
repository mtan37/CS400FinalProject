...
public class Main extends Application implements EventHandler<ActionEvent> {
	...
	/**
	 * Generate a saving address under application/savedQuiz for the current unsaved
	 * quiz
	 * 
	...
	private Scene setUpAddQuestionPage() {
		...
		Label imageLabel = new Label("Image File Name");
		...
		finishAddChoiceBt.setOnAction(event -> finishAddChoiceBt(choiceFields, choicesDescripList, buttonBox,
		...
		goBackToChoiceBt(choicesBox2, choiceBoxList, buttonBox, finishAddChoiceBt, saveButton, choicePrompt));

		saveButton.setOnAction(event -> {
			saveFunction(topicField, descriptionArea, imageField, choicesBox, choiceLabel, choicePrompt);
		});
	 ...
	 * @param choicePrompt
	 */
	private void goBackToChoiceBt(VBox choicesBox2, HBox[] choiceBoxList, HBox buttonBox, Button finishAddChoiceBt,
			Button saveButton, Label choicePrompt) {
		...
		choicesBox2.getChildren().addAll(choiceBoxList[0], choiceBoxList[1], choiceBoxList[2], choiceBoxList[3],
				choiceBoxList[4]);
	 ...
	 * @param choiceBoxList
	 */
	private void finishAddChoiceBt(TextField[] choiceFields, ArrayList<String> choicesDescripList, HBox buttonBox,
			Button goBackToChoiceBt, Label choicePrompt, Button saveButton, VBox choicesBox2, HBox[] choiceBoxList) {
		...
		if (choicesDescripList.size() > 1) {
			...
			for (int i = 0; i < choicesBox2.getChildren().size(); i++) {
				...
				button.setOnMouseClicked(events -> {
					// Make all the buttons to display default text and color
					for (int j = 0; j < choicesBox2.getChildren().size(); j++) {
						...
						((ToggleButton) ((HBox) choicesBox2.getChildren().get(j)).getChildren().get(1)).getStyleClass()
								.remove("redButton");
						((ToggleButton) ((HBox) choicesBox2.getChildren().get(j)).getChildren().get(1)).getStyleClass()
								.remove("greenButton");
						((ToggleButton) ((HBox) choicesBox2.getChildren().get(j)).getChildren().get(1)).getStyleClass()
					...
					// Now correct answer is chosen
					correctAnsSelected = true;
					
					// Change the color and text of chosen button
		...
		// When the user didn't enter more than two choice textFields, display warning
		else {
	 ...
	 * @param choicePrompt
	 */
	private void saveFunction(TextField topicField, TextArea descriptionArea, TextField imageField, VBox choicesBox, Label choiceLabel,
			Label choicePrompt) {
		...
		// If the image field is empty
			...
			Alert alert = new Alert(AlertType.INFORMATION, "Please enter your image file before proceeding");
			alert.initModality(Modality.APPLICATION_MODAL);
			alert.initOwner(primaryStage);
			alert.showAndWait().filter(response -> response == ButtonType.OK);
		}
		...
		// If the user entered all the required field
			...
			save.setOnMouseClicked(events -> {
				...
				// Find the correct choice and set the index of that choice to new question
				...
				// Add the created question to the data structure
