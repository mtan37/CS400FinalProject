...
public class Main extends Application implements EventHandler<ActionEvent> {
  ...
  private Scene setUpAddQuestionPage() {
        ...
        TextField[] choiceFields = new TextField[]{choiceField1, choiceField2,choiceField3,choiceField4,choiceField5};
        ...
        HBox choiceBox1 = new HBox();
        HBox choiceBox2 = new HBox();
        HBox choiceBox3 = new HBox();
        HBox choiceBox4 = new HBox();
        HBox choiceBox5 = new HBox();
        choiceBox1.getChildren().addAll(choiceField1);
        choiceBox2.getChildren().addAll(choiceField2);
        choiceBox3.getChildren().addAll(choiceField3);
        choiceBox4.getChildren().addAll(choiceField4);
        choiceBox5.getChildren().addAll(choiceField5);
        HBox[] choiceBox = new HBox[]{choiceBox1, choiceBox2,choiceBox3,choiceBox4,choiceBox5};
        choicesBox.getChildren().addAll(choiceLabel, choicePrompt,choiceBox1,choiceBox2,choiceBox3,choiceBox4,choiceBox5);
		...
		finishAddChoiceBt.setOnAction(event -> {
		  ...
		  for(int i = 0; i < 5; i++) {
			...
			if (choicesDescripList.size() > 1) {
				...
				choicesBox.getChildren().clear();//clear choices box
				...
				for (int i = 0; i < choiceFields.length; i++) {
				  if(choiceFields[i].getText().trim().compareTo("") != 0) {//how to detect no entry TODO
				    choicesBox.getChildren().add(choiceBox[i]);
				  }
				}
			  ...
			  Alert alert =
		          new Alert(AlertType.INFORMATION, "Please enter at least two choices");
		      alert.initModality(Modality.APPLICATION_MODAL);
		      alert.initOwner(primaryStage);
		      alert.showAndWait().filter(response -> response == ButtonType.OK);
		      return;
  ...
  private Scene setUpQuestionFilterPage() {
    ...
    ObservableList<String> cmLs =
        FXCollections.observableArrayList("BST", "HashTable", "AVL-Tree", "Graph", "Linux", "Set");
        ...
        CheckBoxListCell.forListView(new Callback<String, ObservableValue<Boolean>>() {
          ...
          public ObservableValue<Boolean> call(String item) {
            ...
            observable.addListener((obs, wasSelected, isNowSelected) -> {
              System.out.println("Check box for " + item + " changed from " + wasSelected + " to "
                  + isNowSelected);
